...
import com.k2.mobile.app.model.adapter.LivelyConsultationAdapter;
...
*/
@SuppressLint("NewApi")
public class HRHelperActivity extends BaseActivity implements IXListViewListener, OnClickListener, OnItemClickListener {
	...
	private LivelyConsultationAdapter lcAdapter = null;
		...
		public void dispatchMessage(Message msg) {
			switch(msg.what){
				case 1:
					...
					if(null != json){
						hList.clear();
						hList.addAll(JSON.parseArray(json, PopularTypeBean.class));
						initAdapter();
					}
	...
	private void initView() {
		...
		tv_title.setText(getString(R.string.hr_assistant));	
	...
	private void logic(){
		...
		hList = new ArrayList<PopularTypeBean>();
		// 获取用户信息
		String info = getMenu();
		if (null != info) {
			SendRequest.submitRequest(HRHelperActivity.this, info, submitCallBack);
		}
	}

	private void initAdapter(){
		lcAdapter = new LivelyConsultationAdapter(this, hList);
	...
	private String getMenu(){
		...
		bBean.setInvokeFunctionCode("F50000001");
	...
	RequestCallBack<String> submitCallBack = new RequestCallBack<String>() {
		...
		@Override
		public void onStart() {
			DialogUtil.showWithCancelProgressDialog(HRHelperActivity.this, null, res.getString(R.string.global_prompt_message), null);
		}
		...
		public void onSuccess(ResponseInfo<String> responseInfo) {                                     
			...
			if (null != result && !"".equals(result.trim())) {
				...
				if (null == resMsg || null == resMsg.getResHeader().getStateCode() || "".equals(resMsg.getResHeader().getStateCode())) {				
					LogUtil.promptInfo(HRHelperActivity.this, ErrorCodeContrast.getErrorCode("0", res));
					return;
				// 验证不合法
				}else if (!"1".equals(resMsg.getResHeader().getStateCode())) {
					LogUtil.promptInfo(HRHelperActivity.this, resMsg.getResHeader().getReturnMsg());	
					return;
				}
				// 判断消息体是否为空
		...
		public void onFailure(HttpException error, String msg) {
			...
			if(null != msg && msg.equals(LocalConstants.API_KEY)){
				LogUtil.promptInfo(HRHelperActivity.this, ErrorCodeContrast.getErrorCode("1207", res));
			}else{
				LogUtil.promptInfo(HRHelperActivity.this, ErrorCodeContrast.getErrorCode("0", res));
			}
		}
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		PopularTypeBean bean = hList.get(position - 1);
		Intent mIntent = new Intent(HRHelperActivity.this, LConsultationActivity.class);
		mIntent.putExtra("title", bean.getTypeName());
		mIntent.putExtra("typeCode", bean.getTypeCode());
