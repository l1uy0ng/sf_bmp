 ...
 * @version V1.0
 */
public class NewsProcessListAdapter extends BaseAdapter {

	private Context context;
	private List<CommonNewsBean> newsList;
	
	public NewsProcessListAdapter(Context context, List<CommonNewsBean> newsList) {
		super();
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null){
			...
			convertView = LayoutInflater.from(context).inflate(R.layout.item_news_process_list, null);
		...
		viewHolder.tv_news_tit.setText(bean.getFormTitle());
		viewHolder.tv_author_dep.setText("（"+bean.getRealityOrgName()+"）");
		viewHolder.tv_author.setText(bean.getCreateUserChsName());
		if(null != bean.getCreateDateTime() && !"".equals(bean.getCreateDateTime())){
			String time = bean.getCreateDateTime().replaceAll("T", " ");
	...
	protected class ViewHolder {
		TextView tv_news_tit, tv_author_dep, tv_author, tv_time;
	}
}
