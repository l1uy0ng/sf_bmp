...
public class MyTaskAdapter extends BaseAdapter {
	...
	public MyTaskAdapter(Context _mContext, List<ResTaskBean> _beanList, int _taskType) {
		super();
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		case 0:
			if (null != process && "0".equals(process.trim())) {
				holder.be_accepted.setText(mContext.getResources().getString(R.string.to_be_accepted));
			...
			if (null != itemStatus && "0".equals(itemStatus.trim())) {
				...
				holder.termination.setOnClickListener(new OnClickListener() {
					@Override
					public void onClick(View v) {
						String info = updateTaskInfo(bean.getId(), 0);	// 开始0, 完成1 终止2
				...
				if (null == msg.getMessage() || "".equals(msg.getMessage().trim())) {		 
					...
					if (null != decode) {
						...
						if(null != bean && null != bean.getSuccess() && "1".equals(bean.getSuccess())) {
							String action = BroadcastNotice.TASK_UI_UPDATE;
							if (1 == taskType) {
								action = BroadcastNotice.TASK_UNFINISH_UPDATE;
							}else if (2 == taskType) {
								action = BroadcastNotice.TASK_FINISH_UPDATE;
							}
